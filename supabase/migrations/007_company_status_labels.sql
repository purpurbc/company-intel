-- Keep status labels available after importing new company statuses.
BEGIN;
...
SELECT
  ...
  COALESCE(cs.name, c.company_status) AS company_status_name,
...
SELECT
  ...
  COALESCE(cs.name, c.company_status) AS company_status_name_dim,
